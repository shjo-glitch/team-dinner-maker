...
2. **일정 & 장소 정하기** — 날짜 투표와 '어디서 볼까?'의 후보지 등록·투표를 **병행**하고, 모임 주최자가 상단 스위치로 **일정 확정**(날짜·시간)과 **장소 확정**을 각각 켭니다
...
| 팀즈로 공유 | **일정·장소가 모두 확정된 3단계에서만**, '어디서 볼까?'의 **확정된 후보지 카드에** 파란 ↗ 공유 버튼이 나타납니다. ① **채널 골라서 보내기** — Teams 공유 창에서 채널·채팅을 직접 골라 보냅니다(사전 설정 없음). ② **자동 게시(고급)** — 웹후크 URL을 넣으면 확정된 일정·장소 카드가 자동으로 올라갑니다. 서버도 미확정 상태의 공유 요청은 `409`로 거부합니다. |
| 링크 미리보기 | `/m/:id` 응답에 약속별 **OG 메타태그**(제목·참여 현황·날짜 1위·장소 1위)를 주입해, Teams를 비롯한 메신저에서 링크가 **내용 있는 카드**로 펼쳐집니다. |
| 일정 확정 | 상단 **일정 확정하기 스위치**(OFF→ON)에서 **만나는 날짜**(1위 후보 날짜만 선택 가능)와 **시간**, 관리 비밀번호를 입력합니다. ON 상태에서 다시 누르면(**일정 다시 정하기**) 확정이 풀립니다. 장소 확정과는 **독립**이라 서로 영향을 주지 않고, 날짜·시간 값은 보존되어 재확정 때 채워집니다. |
| 장소 확정 | **일정 확정 전에도** '어디서 볼까?'에서 후보지 등록·투표·확정이 가능합니다(병행). 후보지 카드의 **이 곳으로 확정하기** 또는 상단 **장소 확정하기 스위치**(득표순 후보 선택)로 정하고, **네이버 지도 링크(선택)** 를 넣으면 팀즈 공유에 정확한 POI 링크가 실립니다. ON 상태에서 다시 누르면(**장소 다시 정하기**) 확정이 풀리고, 확정된 후보지를 삭제해도 풀립니다. |
...
팀즈 공유는 **일정과 장소가 모두 확정된 뒤에만** 할 수 있습니다.
...
일정과 장소는 상단의 **스위치 두 개**로 각각 확정합니다. (OFF→ON: 확정, ON→OFF: 다시 정하기 — 둘 다 관리 비밀번호 필요)

### 6-1. 일정 확정하기

상단의 **일정 확정하기** 스위치를 누릅니다.
...
4. 확정되면 스위치가 ON(초록)이 되고 '어디서 볼까?' 화면으로 이동합니다. ON 상태의 라벨은 **일정 다시 정하기**로 바뀝니다.
...
상단의 **장소 확정하기** 스위치로도 정할 수 있어요 — 이때는 **득표순 후보 목록**에서 고릅니다. (일정 확정 전에도 가능)
