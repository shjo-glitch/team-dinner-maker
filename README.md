...
| 겹침 농도 뱃지 | 날짜별로 가능한 인원이 많을수록 뱃지가 진해집니다. 뱃지 색상은 `#7A1025`(빨간색)이며, **1명이 선택할 때마다 알파값이 (1/참여자 수)씩** 더해져 전원이 겹친 날은 완전히 진하게 표시됩니다. |
| 최다 겹침 강조 | 가장 많은 인원이 겹친 날(공동 1등 포함)은 뱃지 **바깥쪽에 노란 테두리**가 표시됩니다. |
| 일정 수정/삭제 | 같은 이름으로 다시 저장하면 일정이 수정되고, 참여자 칩의 ✕로 삭제할 수 있습니다. |
...
- **총 인원** : 팀 전체 인원수. 약속 페이지의 참여 현황(참여 n명 / 총원 m명) 표시에 사용됩니다.
...
**📅 내 일정 등록** 탭에서 이름을 입력하고, 달력에서 가능한 날짜를 선택합니다.
...
아래는 9/28에서 다음 달 10/8까지 **달 경계를 넘어 드래그하는 중**의 화면입니다.
범위 미리보기가 연한 색으로 표시되고, 테마가 "평일 약속"이라 주말은 흐리게 비활성화되어
...
날짜 선택 없이 이름만 먼저 등록하고 싶다면 이름 입력란에서 **엔터**를 치세요 — 참여자로 바로 등록되고,
등록 탭에 남아 이어서 날짜를 선택할 수 있습니다.
...
일정을 수정하려면 **같은 이름을 다시 입력**하거나 참여자 목록에서 자기 이름 칩을 클릭해 기존 선택을 불러온 뒤,
날짜를 고쳐서 다시 저장하면 됩니다. 칩의 ✕ 버튼으로 참여자를 삭제할 수도 있습니다.

## 4. 결과 보기

**📊 결과 보기** 탭에서 날짜별로 몇 명이 가능한지 확인합니다.

- 가능 인원이 있는 날짜는 `#7A1025` 색 뱃지로 표시되며, **1명이 선택할 때마다 (1/참여자 수)만큼 진해집니다.**
  아래 화면은 참여자 5명 기준으로 1명(연함) ~ 5명(완전히 진함)이 겹친 모습입니다.
- **가장 많이 겹친 날(공동 1등 포함)은 뱃지 바깥쪽 노란 테두리**로 강조됩니다. (아래에서 5명이 겹친 9/3)
...
| `PUT` | `/api/events/:id/participants` | 참여자 일정 등록/수정 `{name, dates: ["YYYY-MM-DD", ...]}` (이름 기준 upsert) |
